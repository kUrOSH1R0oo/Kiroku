...
class ServerGUI:
    def __init__(self, root):
        ...
        ttk.Label(self.control_frame, text="Log Level:").pack(pady=5)
        self.log_level_combo = ttk.Combobox(self.control_frame, values=["DEBUG", "INFO", "WARNING", "ERROR"], state="readonly")
        self.log_level_combo.current(1)
        ...
        self.port_spinbox.bind("<Return>", self.update_port)
    ...
    def start_server(self):
        if not self.server_running:
            ...
            self.server_thread = threading.Thread(target=run_flask, args=(self.server_port,), daemon=True)
    ...
    def change_log_level(self, event=None):
        ...
        log_level_str = self.log_level_combo.get()
        log_level_filter = getattr(logging, log_level_str)
        logger.setLevel(log_level_filter)
        self.log(f"Log level set to {log_level_str}.")

    def update_port(self, event=None):
        try:
            self.server_port = int(self.port_spinbox.get())
            self.log(f"Server port updated to {self.server_port}.")
        except ValueError:
